...
void SQLiteDataSource::fillContactFromRow(sqlite3_stmt *s, Contact& c)
{
    ...
    c.email = reinterpret_cast<const char*>(sqlite3_column_text(s, 5));
     ...
     c.state = reinterpret_cast<const char*>(sqlite3_column_text(s, 7));
}
...
ErrorInfo SQLiteDataSource::addContact(const Contact& c)
{
    ...
    sqlite3_bind_text(insertStatement.get(), 6, c.city.c_str(), -1, SQLITE_STATIC);
     sqlite3_bind_text(insertStatement.get(), 7, c.state.c_str(), -1, SQLITE_STATIC);
...
ErrorInfo SQLiteDataSource::updateContact(Contact::ContactId id, const Contact& c)
{
    ...
    sqlite3_bind_text(updateStatement.get(), 7, c.state.c_str(), -1, SQLITE_STATIC);
